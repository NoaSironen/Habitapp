...
export default class ChangeUserPassword extends Component {
  ...
  // Check's if the password inputs have the same values and if true, 
  // changes the password in Firebase authentification database. 
  changePassword = () => {
    ...
    if (newPassword.length < 6) {
      ...
      if (this.state.newPassword === this.state.repetedNewPassword) {
        user.updatePassword(newPassword).then(function () {
          ...
          // If it was more than 5 mins ago since last log in, the user is navigated to log in screen.
    ...
    return (
      <KeyboardAvoidingView behavior='padding' style={styles.container}>
        ...
        <TextInput style={styles.inputStyle}
          ...
          onSubmitEditing={() => this.repetedNewPasswordInput.focus()}
          onChangeText={
            (text) => {
        ...
        <TextInput style={styles.inputStyle}
          ...
          secureTextEntry={true}
          ref={(input) => this.repetedNewPasswordInput = input}
          onChangeText={
            (text) => {
